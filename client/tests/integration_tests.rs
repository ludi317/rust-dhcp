...
async fn test_dhcp_configure_dora_sequence() {
    ...
    let mut client = create_test_client("rust-test-dora").await
        .expect("Failed to create test client");
    
    println!("Testing DHCP DORA sequence...");
    
    // Test full DORA sequence
    let config = timeout(Duration::from_secs(30), client.configure()).await
...
async fn test_dhcp_init_reboot() {
    ...
    let mut client = create_test_client("rust-test-reboot").await
        .expect("Failed to create test client");
    
    println!("Testing DHCP INIT-REBOOT sequence...");
    
    // First, get an initial lease
    let initial_config = timeout(Duration::from_secs(30), client.configure()).await
    ...
    let mut reboot_client = create_test_client("rust-test-reboot").await
